...
export function bundle(time: Date | number, ...packets: Uint8Array[]) {
  ...
  for (let packet of packets) {
    size += 4; // Packet size descriptor
    size += packet.length;
  }

  // Check that size is a multiple of 4
  if (size % 4 !== 0) {
    throw Error(`An OSC bundle must have a size that's a multiple of 4.`);
  }

  // Allocate bundle data
  const bundle = new Uint8Array(size);

  // This DataView offers a better interface for writing multi-byte numbers
  // to the same data buffer
  const bundleDataView = new DataView(bundle.buffer);

  // Write bundle header
  let offset = 0;
  bundle.set(toOSCString('#bundle'), offset);
  offset += 8; // toOSCString adds one null character to the length
  ...
  // Write packets
  for (let packet of packets) {
    bundleDataView.setInt32(offset, packet.length);
    offset += 4;
    bundle.set(packet, offset);
...
export function onMessage(
  callbackOrAddress: (data: any) => any | string,
  callback?: undefined | ((data: any) => any)
) {}
...
// DATA TYPES
function toOSCString(str: string) {
  let unterminatedBuffer = new TextEncoder().encode(str);
  let terminatedLength = (unterminatedBuffer.length + 4) & ~0x03;
...
//   osc.isCommonJS =
//     typeof module !== 'undefined' && module.exports ? true : false;
...
//       : typeof util !== 'undefined' && typeof (util.TextDecoder !== 'undefined')
...
//       : typeof util !== 'undefined' && typeof (util.TextEncoder !== 'undefined')
...
//       (typeof buf.length === 'undefined' || typeof buf === 'string')
//     ) {
//       throw new Error(
...
//       throw new Error(
//         'A malformed type tag string was found while reading ' +
//           'the arguments of an OSC message. String was: ' +
//           typeTagString,
//         ' at offset: ' + offsetState.idx
//       );
//     }
...
//   // Unsupported, non-API function.
//   osc.readArgumentsIntoArray = function (
//     arr,
//     argTypes,
//     typeTagString,
//     dv,
...
//         if (endArrayIdx < 0) {
//           throw new Error(
//             "Invalid argument type tag: an open array type tag ('[') was found " +
//               "without a matching close array tag ('[]'). Type tag was: " +
//               typeTagString
//           );
//         }

//         var typesInArray = fromArrayOpen.slice(0, endArrayIdx);
//         arg = osc.readArgumentsIntoArray(
//           [],
//           typesInArray,
//           typeTagString,
//           dv,
...
//     if (address.indexOf('/') !== 0) {
//       throw new Error(
//         'A malformed OSC address was found while reading ' +
//           'an OSC message. String was: ' +
//           address
//       );
//     }
...
//         collector = packet.address
//           ? osc.collectMessageParts
//           : osc.collectBundlePackets,
//         packetCollection = collector(packet, options);

//       dataCollection.byteLength += packetCollection.byteLength;
//       osc.addDataPart(
//         osc.writeInt32(packetCollection.byteLength),
//         dataCollection
//       );
//       dataCollection.parts = dataCollection.parts.concat(
//         packetCollection.parts
//       );
//     }
...
//       throw new Error(
//         "An OSC bundle must contain 'timeTag' and 'packets' properties. " +
//           'Bundle was: ' +
...
//     throw new Error(
//       "The header of an OSC packet didn't contain an OSC address or a #bundle string." +
//         ' Header was: ' +
//         header
//     );
//   };
...
//     } else {
//       throw new Error(
//         'The specified packet was not recognized as a valid OSC message or bundle.' +
//           ' Packet was: ' +
...
//     throw new Error(
//       "Can't infer OSC argument type for value: " + JSON.stringify(arg, null, 2)
//     );
//   };
